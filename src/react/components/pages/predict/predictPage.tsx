...
import RenderJson from "./../prebuiltPredict/RenderJson";
...
export interface IPredictPageState extends ILoadFileResult, ITableState {
    ...
    showRenderedDocument?: boolean;
    togglePredictBlock?: boolean
}
...
export default class PredictPage extends React.Component<IPredictPageProps, IPredictPageState> {
    ...
    saveAsExcel = async () => {
        const wb = new ExcelJS.Workbook();
            const analyzeResult = this.state.analyzeResult;
            ...
            for (let i = 0; i < ocrPageResults.length; i++) {
                ...
                let ws = wb.addWorksheet(`Page ${currentPageResult.page}`,{properties:{tabColor:{argb:'FFC0000'}}});
                        ...
                        if (table.cells && table.columns && table.rows) {
                            for (let i = 0; i < table.rows; i++) {
                                ...
                                for (let j = 0; j < table.columns; j++) {
                                    let cell = table.cells.find((c) => { return c.rowIndex == i && c.columnIndex == j });
                                    if (cell){
                                        tableRow.push(cell.text)
                                    }else
                                        tableRow.push("")
                                }
                                if(tableRow.find(function(v){ return v}))
                                    var row = ws.addRow(tableRow);
                            }
                        }
                    });
                }
                const documentResults = analyzeResult["documentResults"];
                let docRes = documentResults.find(function(doc){ return doc.pageRange && doc.pageRange.includes(currentPageResult.page)})
                if(docRes && docRes.fields){
                    var emptyRow = ws.addRow([""]);
                    let fields:any = Object.values(docRes.fields);
                    fields.forEach((para, index) => {
                        if(para.page === currentPageResult.page){
                            var paraRow = ws.addRow([""]);
                            ws.mergeCells(`A${paraRow._number}:D${paraRow._number}`);
                            ws.getCell(`A${paraRow._number}`).value = para.text;
                            var emptyRow = ws.addRow([""]);
                        }
                    });
                }
        ...
        saveAs(new Blob([buf]), 'excelExport.xlsx');
    }
    ...
    public render() {
        ...
        const onPredictionPath: boolean = this.props.match.path.includes("predict") || this.props.match.path.includes("custom-predict");
        ...
        return (
            <div
                ...
                {this.state.analyzeResult&& !this.state.isPredicting && <div className="bg-lighter-1" id="extractedContent" >
                        <div className="toggle-btn-block bg-lighter-1 text-dark">
                            <FontIcon className="mr-1 toggle-icon" iconName={this.state.showRenderedDocument ? "ChevronRight" : "ChevronLeft"} onClick={() => {
                                this.setState({ showRenderedDocument: !this.state.showRenderedDocument })
                            }} />
                        </div>
                        {this.state.showRenderedDocument && <div className="renderedData p-2" style={{ width: (window.innerWidth / 2)-150 }}>
                            ...
                            <Tabs>
                                ...
                                <TabPanel>
                                    <RenderJson data={{analyzeResult:this.state.analyzeResult}} currentPage={this.state.currentPage} />
                                </TabPanel>
                                <TabPanel>
                                    {this.state.analyzeResult && !this.state.isPredicting &&
                                        <div className="container-items-center container-space-between results-container">
                                            <ReactJson src={this.state.analyzeResult} collapsed={true}/>
                                        </div>
                                    }
                                </TabPanel>
                            </Tabs>
                ...
                <div id="rightSection">
                <div className="toggle-btn-block bg-lighter-1 text-dark">
                            <FontIcon className="mr-1 toggle-icon bg-lighter-1" iconName={this.state.togglePredictBlock ? "ChevronRight" : "ChevronLeft"} onClick={() => {
                                this.setState({ togglePredictBlock: !this.state.togglePredictBlock })
                            }} />
                        </div>
                        {!this.state.togglePredictBlock && <div className={"predict-sidebar bg-lighter-1"} style={{ width: sidebarWidth, minWidth: sidebarWidth }}>
                                                    ...
                                                    <tbody>
                                                        <tr>
                                                            <td className="model-selection-info-header p-0" >
                                                                <span className="model-selection-info-key text-dark">
                                                                    {strings.predict.modelIDPrefix}
                                                                </span>
                                                                <span title={mostRecentModel.modelInfo.modelId} className="model-selection-info-value text-dark">
                                                        ...
                                                        <tr>
                                                            {mostRecentModel.modelInfo.modelName &&
                                                                <td className="model-selection-info-header p-0" >
                                                                    ...
                                                                    <span title={mostRecentModel.modelInfo.modelName} className="model-selection-info-value text-dark">
                                                                        {mostRecentModel.modelInfo.modelName}
                                                                    </span>
                                                                </td>
                                                            }
    ...
    //---
    private onSelectPagination = (event, option) => {
        this.setState((prevState) => ({
            currentPage: option.key
        }), () => {
            this.imageMap?.removeAllFeatures();
        });
    }
    ...
    private renderPageIndicator = () => {
        ...
        for (let i = 1; i <= numPages; i++) {
            opts.push({ key: i, text: i.toString() })
        }
        return numPages > 1 ? <p className="dynamic-pagination text-orange font-weight-bold"> <span className="page-msg">Page &nbsp; <Dropdown
            className="sourceDropdown"
            selectedKey={this.state.currentPage}
            options={opts}
            disabled={false}
            onChange={this.onSelectPagination}
        /> </span><span className="page-msg">Of {numPages} </span> </p> : <div></div>
    ...
    private runAnalysisCall = (pn?:any, limit?:any) => {
        this.setState({ predictionLoaded: false, isPredicting: true });
        const { numPages } = this.state;
        this.getPrediction(pn, limit)
            .then((result) => {
                let resp =  _.cloneDeep(result);
                if((this.analyzeResults) && (pn != undefined) && (pn != 1)){
                    var t = this.analyzeResults.analyzeResult;
                    let t2 = {"pageResults":[...t.pageResults, ...resp.analyzeResult.pageResults],
                        "documentResults":[...t.documentResults, ...resp.analyzeResult.documentResults],
                        "readResults":[...t.readResults, ...resp.analyzeResult.readResults],
                    }
                    this.analyzeResults.analyzeResult =  t2;
                    
                }else{
                    this.analyzeResults =  resp;
                }
                if(this.state.analyzeResult && (pn != undefined) && (pn != 1)){
                    var t = this.state.analyzeResult;
                    let t2:any = {"pageResults":[...t.pageResults, ...resp.analyzeResult.pageResults],
                        "documentResults":[...t.documentResults, ...resp.analyzeResult.documentResults],
                        "readResults":[...t.readResults, ...resp.analyzeResult.readResults],
                    };
                    this.tableHelper.setAnalyzeResult(t2);
                    this.setState({
                        predictionResult: this.analyzeResults,
                        analyzeResult: t2,
                        predictionLoaded: true,
                        predictRun: true,
                        isPredicting: false,
                        togglePredictBlock:!this.state.togglePredictBlock,
                        showRenderedDocument:true
                    }, () => {
                        this.drawPredictionResult();
                    });
                }else{
                ...
                if(pn && limit<numPages){
                    this.runAnalysisCall(limit+1, limit+100);
                }
            })
            .catch((error) => {
                console.log(error,"==");
    ...
    private handleClick = () => {
        const { numPages } = this.state;
        if(numPages>200){
            this.runAnalysisCall(1,100);
        }else{
            this.runAnalysisCall();
        }
...
// }
    ...
    private async getPrediction(pn?:any, limit?:any): Promise<any> {
        const modelID = this.props.project?.recentModelRecords?.[this.state.selectedRecentModelIndex].modelInfo.modelId;
        const { numPages } = this.state;
        if (!modelID) {
            throw new AppError(
    ...
    private drawPredictionResult = (): void => {
        ...
        const ocrExtent = [0, 0, ocrForCurrentPage ? ocrForCurrentPage.width :0, ocrForCurrentPage ? ocrForCurrentPage.height: 0];
