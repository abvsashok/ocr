...
import { getPrimaryWhiteTheme, getWhiteOrangeTheme } from "../../../../common/themes";
...
export class LayoutPredictPage extends React.Component<Partial<ILayoutPredictPageProps>, ILayoutPredictPageState>{
    ...
    componentDidUpdate(_prevProps: ILayoutPredictPageProps, prevState: ILayoutPredictPageState) {
        console.log(this.state.currentPage, this.state, "=======")
        if (this.state.file) {
    ...
    render() {
        ...
        let widthPer = this.state.togglePredictBlock ? 0.75 : 1;
        return (
            <>
                <div
                    ...
                    {this.state.layoutData && !this.state.isAnalyzing && <div className="bg-lighter-1" id="extractedContent" >
                        ...
                        {this.state.showRenderedDocument && <div className="renderedData p-2" style={{ width: (window.innerWidth / 2) * widthPer}}>
                            ...
                            <Tabs>
                                ...
                                <TabPanel>
                                    <RenderJson data={this.state.layoutData} currentPage={this.state.currentPage} />
                                </TabPanel>
                                <TabPanel>
                                    {this.state.layoutData && !this.state.isAnalyzing &&
                                        <div className="container-items-center container-space-between results-container">
                                            <ReactJson src={this.state.layoutData} collapsed={true}  />
                                        </div>
                                    }
                                </TabPanel>
                            </Tabs>
                    ...
                    <div id="rightSection">
                        ...
                        {this.state.togglePredictBlock && <div className="predict-sidebar bg-lighter-1">
    ...
    fillCellColor = (row, properties) => {
        row.eachCell({ includeEmpty: true }, function (cell, v) {
            cell.font = properties.font;
            cell.fill = properties.fill;
        })
    }
    saveAsExcel = async () => {
        ...
        if (layoutData) {
            ...
            for (let i = 0; i < ocrPageResults.length; i++) {
                ...
                let ws = wb.addWorksheet(`Page ${currentPageResult.page}`,{views: [{showGridLines: false}],properties:{tabColor:{argb:'FFC0000'},defaultRowHeight:20}});
                        ...
                        if (table.cells && table.columns && table.rows) {
                            for (let i = 0; i < table.rows; i++) {
                                ...
                                if(tableRow.find(function(v){ return v})){
                                    ...
                                    if(i==1 || i==2){
                                        this.fillCellColor(row, excelBg['orange']);
                                    }else if(i>2 && i%2==0){
                                        this.fillCellColor(row, excelBg['gray']);
                                    }
                ...
                if(docRes && docRes.fields){
                    ...
                    fields.forEach((para, index) => {
                        if(para.page === currentPageResult.page){
                            ...
                            paraRow.border = false;
                            ws.mergeCells(`A${paraRow._number}:D${paraRow._number}`);
                            let ec = ws.getCell(`A${paraRow._number}`);
                            ec.value = para.text;
                            // ec.alignment = {wrapText: true,vertical: 'middle' };
                            var emptyRow = ws.addRow([""]);
                        }
                    });
                }
                //---
                ws.columns.forEach(function(column, v){
                    column.eachCell({ includeEmpty: true }, function(cell,v){
